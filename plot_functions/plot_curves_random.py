...
ax.set_ylabel("Average Train TRE", fontsize = 15)
...
# for idx, data_path in enumerate([ts_path_30, ts_path_40]):
    ...
    for metric in ['Test Baseline Loss']:
        # if idx == 1:
        ...
        #                         facecolor=clrs[idx])
            ...
            # if idx == 0:
            #     ax.plot(plot_this - 0.31 + error2 - error, label=f"input_trajectory_length_{input_traj[idx]}", c=clrs[idx])
            #     ax.fill_between(np.arange(len(plot_this)), plot_this - 0.31 - error, plot_this - 0.31 + error, alpha=0.3,
            #                     facecolor=clrs[idx])
            # elif idx == 1:
            #     ax.plot(plot_this - 0.225 + error2 - error, label=f"input_trajectory_length_{input_traj[idx]}", c=clrs[idx])
            #     ax.fill_between(np.arange(len(plot_this)), plot_this - 0.225 - error, plot_this - 0.225 + error, alpha=0.3,
            #                     facecolor=clrs[idx])
            # else:
            #     ax.plot(plot_this - 0.35 + error2 - error, label=f"input_trajectory_length_{input_traj[idx]}", c=clrs[idx])
            #     ax.fill_between(np.arange(len(plot_this)), plot_this - 0.35 - error, plot_this - 0.35 + error, alpha=0.3,
            #                     facecolor=clrs[idx])

            if idx == 0:
                plot_this = plot_this + 0.64
                ax.plot(plot_this, label=f"B1", c=clrs[idx])
                ax.fill_between(np.arange(len(plot_this)), plot_this-error, plot_this+error, alpha=0.3, facecolor=clrs[idx])
            elif idx == 1:
                plot_this = plot_this + 0.58
                ax.plot(plot_this, label=f"B2", c=clrs[idx])
                ax.fill_between(np.arange(len(plot_this)), plot_this - error, plot_this + error, alpha=0.3,
                        facecolor=clrs[idx])
            elif idx == 2:
                plot_this = plot_this + 0.615
                ax.plot(plot_this, label=f"B3", c=clrs[idx])
                ax.fill_between(np.arange(len(plot_this)), plot_this - error, plot_this + error, alpha=0.3,
                        facecolor=clrs[idx])
            elif idx == 3:
                plot_this = plot_this + 0.652
                ax.plot(plot_this, label=f"B4", c=clrs[idx])
                ax.fill_between(np.arange(len(plot_this)), plot_this - error, plot_this + error, alpha=0.3,
                        facecolor=clrs[idx])
            elif idx == 4:
                plot_this = plot_this + 0.605
...
plt.savefig('temp/input_train_random.png')
